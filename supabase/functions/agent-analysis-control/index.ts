...
function kickWorker(): void {
  ...
  const task = fetch(`${baseUrl}/functions/v1/agent-analysis-worker`, {
    ...
    if (!response.ok) console.error('analysis worker kick failed', response.status);
  }).catch(error => console.error('analysis worker kick failed', error));
...
Deno.serve(async request => {
  ...
  try {
    const input = await request.json() as { companyId?: string; bankAccountId?: string; action?: 'kick' | 'backfill' };
    ...
    if (role === 'viewer') return json({ error: 'Viewers cannot schedule analysis' }, 403);
    let result: { batchId?: string; queued?: number } = {};
    if (input.action === 'backfill') {
      const { data, error } = await service.rpc('enqueue_agent_analysis_backfill', {
    ...
    kickWorker();
    return json({ ...result, scheduled: true }, 202);
  } catch (error) {
    ...
    console.error('agent-analysis-control failed', error);
    return json({ error: error instanceof Error ? error.message : 'Could not schedule analysis' }, 502);
  }
});
