-- A worker isolate can be killed by the platform before it reports its own
-- failure. The finish path already caps retries at three attempts, but the
-- expired-lease sweep in claim_statement_imports returned every silent death
-- to 'retryable', so an extraction that always outlived the worker looped
-- forever, spending a model call per cycle. The sweep now retires imports
-- that have used their attempts, and the claim query refuses to lease them.
...
begin
  ...
  -- Retire any import already past its attempt budget so it can never be
  -- selected again (a backstop for rows stranded by earlier code).
  update public.statement_imports
    set status = 'failed',
