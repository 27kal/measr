...
function Settings({ company, statementImports, workflow, xeroObservation, onChanged, onDeleted, notify }: { company: Company; statementImports: StatementImport[]; workflow: WorkflowState; xeroObservation?: XeroObservationProgress; onChanged: () => Promise<void>; onDeleted: () => Promise<void>; notify: Notify }) {
  ...
  useEffect(() => {
    ...
    void repository.getXeroCandidateOptions(company.id).then(async options => {
      setXeroOptions(options);
      // The server mirrors Xero bank accounts during this read; reload the
      // snapshot when the mirrored list no longer matches what is on screen.
      const synced = options.bankAccounts.map(account => `${account.id}:${account.name}`).sort().join('|');
      const shown = company.bankAccounts.map(account => `${account.xeroAccountId}:${account.name}`).sort().join('|');
      if (synced !== shown) await onChanged();
    }).catch(error => notify(friendlyError(error, 'Could not load Xero accounts'), 'error'));
  ...
  return <div className="content settings-content">
    ...
    <div className={`settings-layout ${operational ? 'settings-layout-single' : ''}`}><div className="settings-main">
      ...
      <section className="settings-card"><div className="settings-card-title"><span className="step-icon"><BuildingLibraryIcon className="ui-icon"/></span><div><h3>Bank data</h3><p>Bank accounts mirror the GBP bank accounts in the connected Xero organisation. Add or rename an account in Xero and it appears here. Upload each bank's statement as CSV, spreadsheet or PDF.</p></div></div>{!draft.setup.xeroConnected ? <p className="bank-sync-empty">Connect Xero to bring in this company's bank accounts.</p> : draft.bankAccounts.length === 0 ? <p className="bank-sync-empty">No GBP bank accounts in the connected Xero organisation yet. Add the account in Xero and it appears here.</p> : draft.bankAccounts.map(account => <div className="bank-source" key={account.id}><div className="bank-row"><div><strong>{account.name}</strong><small>Synced from Xero · Statement uploads · GBP</small></div></div><StatementUploader company={company} account={account} statementImports={statementImports.filter(statementImport => statementImport.bankAccountId === account.id)} workflow={workflow} onChanged={onChanged} notify={notify}/></div>)}</section>
